...
public sealed class TriggerAcceptanceTests {
	...
	public void Running_AlwaysRejectedWithAlreadyRunning() {
		...
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Auto, Now).Rejection.Should().Be(TriggerResult.AlreadyRunning);
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Manual, Now).Rejection.Should().Be(TriggerResult.AlreadyRunning);
	}

	[Fact]
	public void Idle_NoHistory_AutoAndManualAccepted() {
		var inst = MakeInstance(MakeStage(TimeSpan.Zero, RetryPolicy.NoRetry));
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Auto, Now).IsAccepted.Should().BeTrue();
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Manual, Now).IsAccepted.Should().BeTrue();
	}
	...
	public void Terminating_RejectedWithTerminating() {
		...
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Auto, Now).Rejection.Should().Be(TriggerResult.Terminating);
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Manual, Now).Rejection.Should().Be(TriggerResult.Terminating);
	}
	...
	public void Auto_InRetryDelay_ReturnsWaitingRetry() {
		...
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Auto, Now).Rejection.Should().Be(TriggerResult.WaitingRetry);
	}
	...
	public void Manual_InDebounceWindow_AfterSuccess_Rejected() {
		...
		EventLoop.TryAcceptTrigger(inst, TriggerSource.Manual, Now).Rejection.Should().Be(TriggerResult.Debounced);
	}
